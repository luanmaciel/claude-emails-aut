...
## Ponto cego da triagem diária: o que eu mandei e ninguém respondeu

O pré-filtro descarta a thread quando a última mensagem é minha (`filtrar()` →
`ultima_mensagem_minha`), e a rotina diária roda com janela de 3 dias sobre a Inbox. Isso é
correto para a triagem — não adianta reapresentar todo dia um email já respondido —, mas deixa
as cobranças invisíveis: uma thread parada é, por definição, uma em que eu falei por último.

`cobrancas.py` cobre esse ponto cego pelo lado oposto: lê os **Itens Enviados** numa janela
longa (default 60 dias), pareia por `ConversationID` com o que chegou na Inbox **e em suas
subpastas** (a pasta "BioHorizon" fica fora da Inbox raiz) e reporta as threads em que nada
voltou depois do meu envio, ordenadas por dias parados. Roda sob demanda, não no agendador:
...
como falso "respondido" — o erro cai para o lado seguro.
